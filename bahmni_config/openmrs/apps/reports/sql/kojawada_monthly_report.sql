SELECT 'Total number of clinic visits' as Indicator, round(count(*)) AS 'Kojawada'
...
SELECT 'Total no. of patients' as Indicator,round(count(distinct pi.identifier)) AS 'Kojawada'
  FROM patient_identifier pi
  JOIN person p ON p.person_id = pi.patient_id AND pi.voided=0 AND pi.preferred=1
  INNER join visit v on v.patient_id=p.person_id AND v.voided=0 and
  cast(v.date_started AS DATE) between '#startDate#' AND '#endDate#'
...
select 'Number of female %' as Indicator, concat(Female_patient,' (',round((Female_patient / Total_visit)*100,2),')') as Kojawada from (
SELECT 
...
join location l on l.location_id = v.location_id and l.name like '%Kojawada%'
  ...
  INNER join location l on l.location_id=v.location_id and l.name like'%Kojawada%'
  order by v.date_created ) as Kojawada_visit

union all

select 'Number of children under 5 years (%)' as Indicator, concat(cpatient,' (',round((cpatient / Total_visit)*100,2),')') as Kojawada from (
SELECT 
...
union all
 ...
 select 'Number of girls (%)' as Indicator,concat(Female_patient,' (',round((Female_patient / Total_visit)*100,2),')') as Kojawada from (
SELECT 
count(distinct(pi.identifier)) as Female_patient, 
(
...
SELECT 'No. of new patients registered' as Indicator,round(count(distinct pi.identifier))  AS 'Kojawada'
  FROM patient_identifier pi 
  JOIN person p ON p.person_id = pi.patient_id AND pi.voided=0 AND pi.preferred=1 
...
FROM obs o
  ...
  WHERE o.voided is false and l.name like '%Kojawada%'
...
join person p on p.person_id = pi.patient_id and pi.voided = 0 and pi.preferred = 1
join visit v on v.patient_id = p.person_id and v.voided = 0 and cast(v.date_started AS DATE) between '#startDate#' AND '#endDate#'
join location l on l.location_id = v.location_id and l.name like '%Kojawada%'))*100,2),')'
) as Kojawada
  FROM patient_identifier pi 
  JOIN person p ON p.person_id = pi.patient_id AND pi.voided=0 AND pi.preferred=1
  LEFT OUTER JOIN person_address paddress ON p.person_id = paddress.person_id AND paddress.voided=0 
...
round(count(distinct(pi.identifier))) as Kojawada
FROM obs o
...
union all
 ...
 select 'No of ANC with high risk identified' as Indicator, round(count(distinct(formquery.Identifier))) as Kojawada from 
 (
    SELECT
...
select 'Total revenue generated in rupees' as Indicator,round(sum(o.value_numeric)) as Kojawada 
from obs o 
