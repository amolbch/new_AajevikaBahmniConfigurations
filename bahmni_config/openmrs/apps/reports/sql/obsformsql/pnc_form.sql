 ...
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='PNC_Diagnosis', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Diagnosis',
 ...
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='GLOBAL_Name_of_nurse_or_SHW_hindi', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Nurse Name'
    from obs o
     ...
     and o.obs_group_id in (select obs_id from obs where concept_id=4833 and voided is false)
    ...
    JOIN concept_name obs_fscn on o.concept_id=obs_fscn.concept_id AND obs_fscn.concept_name_type="FULLY_SPECIFIED" AND obs_fscn.voided is false
     AND obs_fscn.name IN ('PNC_Diagnosis',"GLOBAL_Hindi_Name_of_Doctor","GLOBAL_Name_of_nurse_or_SHW_hindi")
