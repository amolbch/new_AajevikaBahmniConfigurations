SELECT
   ...
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = '2MT5Y_Heart_Rate', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Heart Rate',
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = '2MT5Y_Breath_Rate', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Breath Rate', 
   ...
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = '2MT5Y_SPO2', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'SpO2',
   ...
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = '2MT5Y_Poshan_stithi', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Poshan ki Stithi',
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = '2MT5Y_Anemia_stithi', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Anemia ki Stithi ',
   ...
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'GLOBAL_Name_of_nurse_or_SHW_hindi', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Nurse Name'
    ...
    and o.obs_group_id in (select obs_id from obs where concept_id=5031 or concept_id=5033 and voided is false)
    ...
    AND obs_fscn.name IN ('2MT5Y_Diagnosis','GLOBAL_Hindi_Name_of_Doctor','GLOBAL_Name_of_nurse_or_SHW_hindi')
...
SELECT
   ...
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'GLOBAL_Name_of_nurse_or_SHW_hindi', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Nurse Name'
     ...
     FROM obs o
    JOIN concept obs_concept ON obs_concept.concept_id=o.concept_id AND obs_concept.retired is false  and o.form_namespace_and_path like '%Child 2 Months to 5 Years Case Sheet%'
    and cast(o.obs_datetime AS DATE) between '#startDate#'AND '#endDate#'
    JOIN concept_name obs_fscn on o.concept_id=obs_fscn.concept_id AND obs_fscn.concept_name_type="FULLY_SPECIFIED" AND obs_fscn.voided is false
    AND obs_fscn.name IN ('GLOBAL_Type_Of_Visit','2MT5Y_Heart_Rate','2MT5Y_Breath_Rate','GLOBAL_Hindi_Temprature','2MT5Y_SPO2','GLOBAL_Hindi_Temprature','GLOBAL_Infant_Weight','GLOBAL_Infant_Height',
    '2MT5Y_Poshan_stithi', '2MT5Y_Anemia_stithi','2MT5Y_Diagnosis',
