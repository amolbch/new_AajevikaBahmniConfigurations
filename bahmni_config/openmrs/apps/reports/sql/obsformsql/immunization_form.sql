 ...
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='Immunization_Done', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Immunization Done',
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='Immunization_Date', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Date Of Immunization',
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='Immunization_Location', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Location'

FROM obs o
    JOIN concept obs_concept ON obs_concept.concept_id=o.concept_id AND obs_concept.retired is false  and o.form_namespace_and_path like '%Immunization Form%'
    ...
    JOIN concept_name obs_fscn on o.concept_id=obs_fscn.concept_id AND obs_fscn.concept_name_type="FULLY_SPECIFIED" AND obs_fscn.voided is false
     AND obs_fscn.name IN ('Immunization_Done','Immunization_Date','Immunization_Location')
