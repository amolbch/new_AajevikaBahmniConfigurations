SELECT
 ...
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='Admission_Date_And _Time', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Admission date and time',
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='Admission_Dis_Date', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Date of discharge',
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='Admission_Diagnosis', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Reason for Admission',
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='Admission_Outcome_of_adm', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Outcome of Admission',
 ...
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='Global_Outcome', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Condition at discharge',
 ...
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='GLOBAL_FollowUpDate', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Follow up on',
 ...
 GROUP_CONCAT(DISTINCT(IF(obs_fscn.name ='GLOBAL_Nurse_Name', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS'Nurses Name'
    ...
    FROM obs o
    JOIN concept obs_concept ON obs_concept.concept_id=o.concept_id AND obs_concept.retired is false  and o.form_namespace_and_path like '%Admission Form%'
    ...
    AND obs_fscn.name IN ('Admission_Date_And _Time','Admission_Dis_Date','Admission_Diagnosis','Admission_Outcome_of_adm','Global_Gnral_Condtion','Global_weight','Temperature','Pulse','RR','SPO2','Systolic','Diastolic','Global_Outcome','Receipt_number',
    'Fees_Amount','GLOBAL_FollowUpDate','GLOBAL_Doctors_Name','GLOBAL_Nurse_Name')
