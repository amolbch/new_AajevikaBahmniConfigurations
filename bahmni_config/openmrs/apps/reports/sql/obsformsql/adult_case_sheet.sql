...
SELECT
   ...
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'ADULT_FollowUP_Gnral_Condtion', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'General Condition',
   ...
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'GLOBAL_Nurse_Name', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Nurse Name'
    ...
    and o.obs_group_id in (select obs_id from obs where concept_id=3944 or concept_id=3947 and voided is false)
    ...
    AND obs_fscn.name IN ('ACS_ADULT_Diagnosis','GLOBAL_Doctors_Name','GLOBAL_Nurse_Name') 
...
SELECT
   ...
   GROUP_CONCAT(DISTINCT(IF(obs_fscn.name = 'GLOBAL_Nurse_Name', coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name), NULL)) ORDER BY o.obs_id DESC) AS 'Nurse Name'
    ...
    and cast(o.obs_datetime AS DATE) between '#startDate#'AND '#endDate#'
    JOIN concept_name obs_fscn on o.concept_id=obs_fscn.concept_id AND obs_fscn.concept_name_type="FULLY_SPECIFIED" AND obs_fscn.voided is false
    AND obs_fscn.name IN ('ADULT_FollowUP_Gnral_Condtion','Global_weight','Global_height','Temperature','Pulse','RR','SPO2','Diastolic','Systolic','ACS_ADULT_Diagnosis','ACS_Sever_Illness',
